...
export default function USPolo() {
  ...
  useEffect(() => {
    fetch('/api/products?gender=Men&brand=U.S.%20Polo')
      .then(res => res.json())
      .then(data => {
        setProducts(data)
        setLoading(false)
  ...
  const shirts = products.filter(p => {
    const cat = Array.isArray(p.category) ? p.category : [p.category]
    return cat.includes('Shirts')
  })
  const jeans = products.filter(p => {
    const cat = Array.isArray(p.category) ? p.category : [p.category]
    return cat.includes('Jeans')
  })

  const categories = [
    { name: 'All', count: products.length },
    { name: 'T-Shirts', count: tshirts.length },
    { name: 'Shirts', count: shirts.length },
          ...
          {categories.map(cat => (
            <button
              ...
              style={{
                ...
                background: activeCategory === cat.name ? '#003087' : '#fff',
        ...
        {!loading && !error && (
          <>
            {(activeCategory === 'All' || activeCategory === 'T-Shirts') && (
              <>
                <div className="section-header"><h2 className="section-title">U.S. Polo T-Shirts</h2></div>
            ...
            {(activeCategory === 'All' || activeCategory === 'Shirts') && (
              <>
                <div className="section-header"><h2 className="section-title">U.S. Polo Shirts</h2></div>
                <div className="product-grid">{shirts.map(p => <ProductCard key={p._id} product={p} />)}</div>
                <div className="divider" />
              </>
            )}
            {(activeCategory === 'All' || activeCategory === 'Jeans') && (
              <>
                <div className="section-header"><h2 className="section-title">U.S. Polo Jeans</h2></div>
