...
export default function VeroModa() {
  ...
  useEffect(() => {
    fetch('/api/products?gender=Women&brand=Vero%20Moda')
      .then(res => res.json())
      .then(data => {
        setProducts(data)
        setLoading(false)
  ...
  const visibleProducts = products.filter(
    (p) => !(p.name === 'Puff Sleeve Dress' && p.image === '/image/women/dress/d4.jpg')
  )

  const dresses = visibleProducts.filter(p => p.category === 'Dresses')
  const tshirts = visibleProducts.filter(p => p.category === 'T-Shirts')
  const jeans = visibleProducts.filter(p => p.category === 'Jeans')
  const displayProducts = [...dresses, ...tshirts, ...jeans]
        ...
        {!loading && !error && (
          <>
            {(activeCategory === 'All' || activeCategory === 'Dresses') && (
              <>
                <div className="section-header"><h2 className="section-title">Vero Moda Dresses</h2></div>
            ...
            {(activeCategory === 'All' || activeCategory === 'T-Shirts') && (
              <>
                <div className="section-header"><h2 className="section-title">Vero Moda T-Shirts</h2></div>
                <div className="product-grid">{tshirts.map(p => <ProductCard key={p._id} product={p} />)}</div>
                <div className="divider" />
              </>
            )}
            {(activeCategory === 'All' || activeCategory === 'Jeans') && (
              <>
                <div className="section-header"><h2 className="section-title">Vero Moda Jeans</h2></div>
