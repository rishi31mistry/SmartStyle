...
export default function BudgetDeals() {
  ...
  useEffect(() => {
    fetch('/api/products?tag=Sale&maxPrice=999')
      .then(r => r.json())
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '24px' }}>
          {filters.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)}
              style={{ padding: '8px 20px', borderRadius: '20px', border: '1.5px solid', borderColor: activeFilter === f ? '#2e7d32' : '#ddd', background: activeFilter === f ? '#2e7d32' : '#fff', color: activeFilter === f ? '#fff' : '#555', fontSize: '13px', fontWeight: '500', fontFamily: "'Poppins', sans-serif", cursor: 'pointer' }}
            >{f}</button>
          ))}
        </div>
              ...
              <div key={p._id} className="product-card"
                ...
                <div className="product-info">
                  ...
                  <div className="product-meta">
                    <div>
                      <span className="product-price" style={{ color: '#2e7d32' }}>₹{p.price.toLocaleString('en-IN')}</span>
                  ...
                  <button className="add-to-cart-btn" style={{ background: '#2e7d32' }}
